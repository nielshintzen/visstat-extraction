formatEflalo2 <- function(x){
  ...
  x$VE_LEN        <- an(x$VE_LEN)
  x$VE_KW         <- an(x$VE_KW)
  x$VE_TON        <- an(x$VE_TON)
  ...
  x$LE_SEQNUM     <- an(x$LE_SEQNUM)
  x$LE_GEAR       <- ac(x$LE_GEAR)
  x$LE_MSZ        <- an(x$LE_MSZ)
  x$LE_RECT       <- ac(x$LE_RECT)
  x$LE_MET_level6 <- ac(x$LE_MET_level6)
  x$LE_UNIT       <- ac(x$LE_UNIT)
  x$LE_EFF        <- an(x$LE_EFF)
  x$LE_EFF_VMS    <- an(x$LE_EFF_VMS)
  for(i in c(grep("_KG_",colnames(eflalo2)),grep("_EURO_",colnames(eflalo2)))) x[,i] <- an(x[,i])
  return(x)
}
