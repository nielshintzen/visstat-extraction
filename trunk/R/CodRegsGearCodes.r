...
input$REG_GEAR <- rep(NA,length(input[,1]))
...
input$REG_GEAR[input$GEAR %in% c("OTTER","DEM_SEINE") & input$MESH_SIZE_RANGE %in% c("100-109","100-119")] <- 'TR1.1'
input$REG_GEAR[input$GEAR %in% c("OTTER","DEM_SEINE") & input$MESH_SIZE_RANGE %in% c(">=120")] <- 'TR1.2'
